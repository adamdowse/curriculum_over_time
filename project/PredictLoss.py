...
plt.scatter(range(100),single_line)
plt.xlabel('epoch')
plt.ylabel('loss')
...
for n in [2,3,5,20,50]:
    for i in range(n,100-1):
        ...
        pred_reg = regressionpred(n,df.iloc[:,3:3+i])
        pred_ransac = ransacpred(n,df.iloc[:,3:3+i])

        #setup df
        if i == n:
            #create the scores df
            scores_nmean = pd.DataFrame(data=testpred(pred_nmean,df.iloc[:,i+1]),columns=[str(i)])
            scores_reg   = pd.DataFrame(data=testpred(pred_reg,df.iloc[:,i+1]),columns=[str(i)])
            scores_ransac= pd.DataFrame(data=testpred(pred_ransac,df.iloc[:,i+1]),columns=[str(i)])
        else:
            scores_nmean = pd.concat([scores_nmean,pd.DataFrame(data=testpred(pred_nmean,df.iloc[:,i+1]),columns=[str(i)])],axis=1)
            scores_reg = pd.concat([scores_reg,pd.DataFrame(data=testpred(pred_reg,df.iloc[:,i+1]),columns=[str(i)])],axis=1)
            scores_ransac = pd.concat([scores_ransac,pd.DataFrame(data=testpred(pred_ransac,df.iloc[:,i+1]),columns=[str(i)])],axis=1)
...
plt.tight_layout()
plt.savefig('predfuncs/curric_predicts')
plt.close()
...
plt.xticks(range(0,100,10))
plt.tight_layout()
plt.savefig('predfuncs/nocurric_predicts')
plt.close()
